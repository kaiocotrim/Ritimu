...
type PlanEvent = { id: string; title: string; startAt: string; endAt: string | null; status: "PENDING" | "COMPLETED" | "CANCELED"; routineType: Kind; recurrence: "NONE" | "WEEKLY" | "CUSTOM"; recurrenceDays: number[]; studySession: { courseId: string | null; subjectName: string | null } | null }
...
export function StudyPlanner({ initialCourses }: { initialCourses: Course[] }) {
  ...
  return <motion.div
    className="study-plan-black-theme mx-auto max-w-5xl text-[#171717]"
    initial={reduceMotion ? false : { opacity: 0, y: 14 }}
    animate={{ opacity: 1, y: 0 }}
    transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }}
  >
    <header className="mb-6 flex flex-col gap-5 text-white lg:flex-row lg:items-end lg:justify-between">
...
function occurrences(events: PlanEvent[], from: Date, to: Date) {
  ...
  for (const source of events) {
    const original = new Date(source.startAt), end = source.endAt ? new Date(source.endAt) : null
    if (source.recurrence === "NONE") { if (original >= from && original < to) result.push({ ...source, startAt: original, endAt: end, source }); continue }
    const weekdays = source.recurrence === "WEEKLY" ? [original.getDay()] : source.recurrenceDays
    for (let date = dayStart(from); date < to; date = addDays(date, 1)) if (date >= dayStart(original) && weekdays.includes(date.getDay())) { const startAt = new Date(date); startAt.setHours(original.getHours(), original.getMinutes()); const span = (end?.getTime() ?? original.getTime()) - original.getTime(); result.push({ ...source, startAt, endAt: new Date(startAt.getTime() + span), source }) }
  }
...
function Loading({ compact = false }: { compact?: boolean }) { return <div className={`grid place-items-center ${compact ? "h-72 rounded-[28px] bg-[#12151D] ring-1 ring-white/[.08]" : "min-h-[70vh]"}`}><LoaderCircle className="size-6 animate-spin text-[#50D05C]" /></div> }
