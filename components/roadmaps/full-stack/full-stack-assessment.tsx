...
import { CharacterRewardRoulette } from "@/components/roadmaps/character-reward-roulette"
...
export function FullStackAssessment({ roadmapId, unlocked, initialResult, userName }: { roadmapId: string; unlocked: boolean; initialResult: Result | null; userName: string }) {
  ...
  return <section className="border-2 border-[#172017] bg-white p-6 shadow-[5px_5px_0_#2878ff] sm:p-8">
    ...
    {result && <div className={`mt-6 border-2 p-5 ${result.passed ? "border-green-700 bg-green-50" : "border-red-700 bg-red-50"}`}><p className="font-pixel text-xl font-bold">{result.correctAnswers}/50 · {result.percentage}%</p><p className="font-pixel mt-2 text-xs font-bold uppercase">{result.passed ? "Aprovado · trilha concluída" : "Reprovado · tente novamente"}</p>{result.passed && <CharacterRewardRoulette roadmapId={roadmapId} />}{result.certificateEligible && <CertificatePanel roadmapId={roadmapId} defaultName={userName} />}</div>}
