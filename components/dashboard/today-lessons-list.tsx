...
import { ArrowRight, BookOpen, Check, Clock3, Dumbbell, Flame, GraduationCap, LoaderCircle, Monitor, Star, Target, Trophy } from "lucide-react"
...
const lessonStyles = {
  ...
  OTHER: { icon: Star, tone: "bg-slate-50 text-slate-700", border: "border-black/10", tag: "Rotina" },
} as const
...
export function TodayLessonsList({ lessons, occurrenceDate, streak }: { lessons: Lesson[]; occurrenceDate: string; streak: number }) {
  ...
  return <>
    <section className="relative mb-10 overflow-hidden rounded-[28px] bg-[#090A0C] px-6 py-7 text-white shadow-xl shadow-black/10 sm:px-8">
      ...
      <div className="relative z-10 max-w-[72%] sm:max-w-[68%]">
        ...
        <div className="mt-6 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-white/70">
          <span className="flex items-center gap-2"><Star className="size-5 fill-amber-300 text-amber-300" /><strong className="text-white">{availableXp} XP</strong> disponíveis</span><span className="hidden h-6 w-px bg-white/25 sm:block" /><span className="flex items-center gap-2"><Flame className="size-5 fill-orange-400 text-orange-400" />Sequência: <strong className="text-white">{streak} dias</strong></span>
        </div>
      </div>
    ...
    <div className="mt-6 grid gap-4 sm:grid-cols-2"><div className="flex items-center gap-4 rounded-3xl border border-black/[0.06] bg-white p-5"><span className="flex size-14 items-center justify-center rounded-2xl bg-amber-50 text-amber-600"><Trophy className="size-7" /></span><div><p className="font-bold">Meta de hoje</p><p className="text-sm text-black/50">Conclua todas as atividades e ganhe</p><p className="font-bold text-amber-600">{availableXp} XP</p></div></div><div className="flex items-center gap-4 rounded-3xl border border-black/[0.06] bg-white p-5"><span className="flex size-14 items-center justify-center rounded-2xl bg-blue-50 text-blue-600"><Target className="size-7" /></span><div><p className="font-bold">Lembre-se</p><p className="text-sm leading-relaxed text-black/50">Consistência é o que te leva longe.<br />Você consegue! 🚀</p></div></div></div>
