...
export async function GET(_request: Request, { params }: RouteParams) {
  ...
  const topics: GoogleClassroomTopic[] = []
  let pageToken: string | undefined

  do {
    const url = new URL(
      `https://classroom.googleapis.com/v1/courses/${encodeURIComponent(courseId)}/topics`
    )
    ...
    const data = (await response.json().catch(() => null)) as
      | (GoogleClassroomTopicsResponse & { error?: { message?: string } })
      | null
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        return Response.json(
          {
            error: "Sem permissão para acessar os tópicos desta turma",
            code: "CLASSROOM_TOPICS_FORBIDDEN",
            details: data?.error?.message,
            action: "Reconecte a conta e conceda acesso aos tópicos.",
          },
          { status: 403 }
        )
      }
      ...
      return Response.json(
        {
          error: "Falha ao buscar tópicos no Google Classroom",
          details: data?.error?.message,
        },
  ...
  return Response.json({ topic: topics })
}
