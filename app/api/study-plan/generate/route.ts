...
export async function POST() {
  ...
  const planDates = [...byDay.keys()].map((date) => new Date(date))
  const oldGoogleEvents = await prisma.calendarEvent.findMany({
    where: { userId, googleEventId: { not: null }, studySession: { status: { in: ["PENDING", "MISSED", "RESCHEDULED"] }, studyPlan: { planDate: { in: planDates } } } },
    select: { googleEventId: true, googleCalendarId: true },
  })
  const googleEvents = new Map<string, Awaited<ReturnType<typeof createStudyPlanGoogleEvent>>>()
  const keyFor = (item: (typeof sessions)[number]) => `${item.id}:${item.scheduledStart.toISOString()}`
  try {
    for (const item of sessions) {
      googleEvents.set(keyFor(item), await createStudyPlanGoogleEvent(userId, {
        title: `Estudo: ${item.title}`,
        description: `${item.subjectName} — ${item.priorityReason}`,
    ...
    await prisma.$transaction(async (tx) => {
      for (const [date, daySessions] of byDay) {
        ...
        for (const item of daySessions) {
          ...
          await tx.calendarEvent.create({ data: { userId, studySessionId: studySession.id, classroomAssignmentId: item.id, title: `Estudo: ${item.title}`, description: `${item.subjectName} — ${item.priorityReason}`, startAt: item.scheduledStart, endAt: item.scheduledEnd, source: "RITIMU", type: "STUDY", status: "PENDING", ...googleEvents.get(keyFor(item))! } })
        }
      }
    })
  } catch (error) {
    await Promise.allSettled([...googleEvents.values()].map((event) => deleteStudyPlanGoogleEvent(userId, event.googleEventId, event.googleCalendarId)))
    return studyPlanGoogleErrorResponse(error)
  }
  await Promise.allSettled(oldGoogleEvents.flatMap((event) => event.googleEventId ? [deleteStudyPlanGoogleEvent(userId, event.googleEventId, event.googleCalendarId ?? undefined)] : []))
