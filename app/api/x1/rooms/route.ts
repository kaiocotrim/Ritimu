...
export async function POST(request: Request) {
  ...
  try {
    ...
    const captureLimit = body && typeof body === "object" && "captureLimit" in body && typeof body.captureLimit === "number" ? body.captureLimit : 3
    const totalRounds = body && typeof body === "object" && "totalRounds" in body && typeof body.totalRounds === "number" ? body.totalRounds : 1
    ...
    const room = await createRoom(userId, { turnTimeSeconds, allowCapture, captureLimit, totalRounds, vsBot, botDifficulty, topicId, subtopic, difficulty })
