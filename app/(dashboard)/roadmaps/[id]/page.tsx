...
export default async function RoadmapPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [roadmap, gamification, missions, activeRoadmaps, recommendations] = await Promise.all([
    prisma.studyRoadmap.findFirst({ where: { id, OR: [{ isPublic: true }, { creatorId: session.user.id }] }, include: { modules: { orderBy: { order: "asc" }, include: { lessons: { orderBy: { order: "asc" }, include: { prerequisites: true, progress: { where: { userId: session.user.id } } } } } } } }),
    ...
    prisma.studyRoadmap.findMany({ where: { isPublic: true, id: { not: id } }, take: 4, orderBy: { title: "asc" }, select: { id: true, title: true, category: true, estimatedHours: true } }),
  ])
  if (!roadmap) notFound()
  const allLessons = roadmap.modules.flatMap((module) => module.lessons)
  const completedIds = new Set(allLessons.filter((lesson) => lesson.progress[0]?.status === "COMPLETED").map((lesson) => lesson.id))
  ...
  return <main className="roadmaps-pixel-ui min-h-screen bg-[#faf9f0] px-4 pb-32 pt-6 text-[#111820] sm:px-7 lg:px-10 xl:pl-[calc(18rem+2.5rem)]"><CharacterSelection /><div className="mx-auto max-w-330"><Link href="/roadmaps" className="font-pixel inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-black/55 transition hover:text-black"><ArrowLeft className="size-4" /> Todos os roadmaps</Link>
    <div className="mt-5 grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_320px]"><div className="min-w-0 space-y-5">
      <header className="relative isolate min-h-56 overflow-hidden rounded-2xl bg-[#09252b] p-7 text-white shadow-sm sm:p-9"><Image src={roadmap.slug === "full-stack-developer" ? "/quadro12.png" : "/loginBanner.png"} alt="" fill priority sizes="(min-width:1280px) 900px,100vw" className="-z-20 object-cover object-center opacity-80" /><div className="absolute inset-0 -z-10 bg-gradient-to-r from-[#071d23] via-[#071d23]/95 to-[#071d23]/10" /><div className="max-w-xl"><p className="font-pixel text-xs font-bold uppercase tracking-wider text-[#54dc67]">{roadmap.category}</p><h1 className="font-pixel mt-2 text-3xl font-bold leading-none tracking-tight sm:text-5xl">{roadmap.title}</h1><p className="mt-3 max-w-lg text-sm leading-6 text-white/75 sm:text-base">{roadmap.description}</p><div className="mt-6 flex flex-wrap items-center gap-6 text-sm"><span className="font-pixel flex items-center gap-2"><Clock3 className="size-4" /> {roadmap.estimatedHours} horas</span><span className="font-pixel flex items-center gap-2"><Star className="size-4 text-amber-300" /> {completedCount}/{allLessons.length} etapas</span></div></div><div className="mt-5 flex items-center gap-4"><div className="h-3 flex-1 overflow-hidden rounded-full bg-white/15"><div className="h-full rounded-full bg-gradient-to-r from-[#50d05c] to-[#84e88d] transition-all" style={{ width: `${percent}%` }} /></div><strong className="font-pixel text-2xl text-[#67e875]">{percent}%</strong></div></header>
      <RoadmapJourney roadmapId={roadmap.id} modules={modules} userName={session.user.name} />
