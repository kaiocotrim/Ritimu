...
import { AlertTriangle, BookOpen, CalendarDays, ChevronLeft, ChevronRight, Cloud, LoaderCircle, Pencil, Plus, Settings, Trash2, X } from "lucide-react"
...
export default function AgendaPage() {
  ...
  async function confirmDelete() {
    ...
    const response = await fetch(`/api/calendar/events/${deleteTarget.id}`, { method: "DELETE" })
    if (!response.ok) {
      const data = await response.json().catch(() => null)
    ...
    setDeleteTarget(null); setDeleting(false); await loadEvents()
  }
  ...
  return <main className="min-h-screen bg-[#f6f5f1] px-5 pb-32 pt-6 text-[#181817] sm:px-8">
    ...
    <AnimatePresence>{deleteTarget && <motion.div role="dialog" aria-modal="true" aria-labelledby="delete-event-title" className="fixed inset-0 z-[60] grid place-items-center bg-black/55 p-4 backdrop-blur-sm" initial={reduceMotion ? false : { opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget && !deleting) setDeleteTarget(null) }}><motion.div initial={reduceMotion ? false : { opacity: 0, y: 22, scale: .96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={reduceMotion ? undefined : { opacity: 0, y: 12, scale: .97 }} transition={{ type: "spring", stiffness: 320, damping: 26 }} className="w-full max-w-md rounded-[28px] bg-white p-6 shadow-2xl sm:p-8"><div className="flex size-14 items-center justify-center rounded-2xl bg-red-50 text-red-500"><AlertTriangle className="size-7" /></div><h2 id="delete-event-title" className="mt-5 text-2xl font-semibold tracking-tight">Excluir evento?</h2><p className="mt-2 text-sm leading-relaxed text-black/50">Você está prestes a excluir <strong className="font-semibold text-black">“{deleteTarget.title}”</strong>. {deleteTarget.syncedWithGoogle ? "Ele também será removido do Google Calendar." : "Essa ação não poderá ser desfeita."}</p><div className="mt-7 grid grid-cols-2 gap-3"><motion.button type="button" disabled={deleting} onClick={() => setDeleteTarget(null)} whileHover={reduceMotion ? undefined : { y: -2 }} whileTap={reduceMotion ? undefined : { scale: .97 }} className="rounded-2xl border border-black/10 px-4 py-3 font-semibold text-black/65 transition hover:bg-black/[.03] disabled:opacity-50">Cancelar</motion.button><motion.button type="button" disabled={deleting} onClick={confirmDelete} whileHover={reduceMotion ? undefined : { y: -2 }} whileTap={reduceMotion ? undefined : { scale: .97 }} className="flex items-center justify-center gap-2 rounded-2xl bg-red-500 px-4 py-3 font-semibold text-white shadow-lg shadow-red-500/20 transition hover:bg-red-600 disabled:opacity-60">{deleting ? <LoaderCircle className="size-4 animate-spin motion-reduce:animate-none" /> : <Trash2 className="size-4" />}{deleting ? "Excluindo…" : "Excluir"}</motion.button></div></motion.div></motion.div>}</AnimatePresence>
    <Sidebar />
  </main>
}
